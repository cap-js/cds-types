...
type BoundEntityClass = Constructable & { kind: 'entity' }
type RequestSubject = Request extends { subject: infer U } ? U : never
type ActionSubject<S> = S extends BoundEntityClass ? S : RequestSubject

// Subtype of Request as used in ActionEventHandlers
type ActionRequest<P, S> = Omit<Request, 'data' | 'subject'> & {
  data: P,
  subject: ActionSubject<S>,
}
